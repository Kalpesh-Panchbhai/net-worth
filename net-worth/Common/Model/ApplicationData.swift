...
struct ApplicationData: Codable {
    ...
    private static func updateChartData() async {
        shared.chartDataList = [String: [ChartData]]()
        await loadChartData()
    ...
    private static func loadChartData() async {
        ...
        for accountData in accountDataList {
            if(accountData.account.accountType.elementsEqual(ConstantUtils.brokerAccountType)) {
                await loadChartDataForBrokerAccount(accountData: accountData)
            } else {
                await loadChartDataForNonBrokerAccount(accountData: accountData)
            }
        }
    }
    
    private static func loadChartDataForBrokerAccount(accountData: AccountData) async {
        let accountInBrokerList = accountData.accountInBroker
        for accountInBroker in accountInBrokerList {
            await loadChartDataForNonExistingBrokerAccount(accountInBroker: accountInBroker)
        }
    ...
    private static func loadChartDataForNonExistingBrokerAccount(accountInBroker: AccountInBrokerData) async {
    ...
    private static func loadChartDataForNonBrokerAccount(accountData: AccountData) async {
        await loadChartDataForNonExistingNonBrokerAccount(accountData: accountData)
    }
    
    private static func loadChartDataForNonExistingNonBrokerAccount(accountData: AccountData) async {
        ...
        for (accountType, accountDataList) in accountDataListByAccountType {
            if(accountType.elementsEqual(ConstantUtils.brokerAccountType) || accountType.elementsEqual("Inactive Account")) {
                if(accountType.elementsEqual("Inactive Account")) {
                    let chartDataListResult = await generateChartdataForMixedAccounts(accountType: "Inactive Account", accountDataList: accountDataList)
                    shared.chartDataList.updateValue(chartDataListResult, forKey: "Inactive Account")
                } else {
                    let chartDataListResult = await generateChartDataForMultipleBrokerAccounts(accountType: ConstantUtils.brokerAccountType, accountDataList: accountDataList)
                    shared.chartDataList.updateValue(chartDataListResult, forKey: ConstantUtils.brokerAccountType)
                }
            } else {
                let chartDataListResult = await generateChartDataForMultipleNonBrokerAccount(accountType: accountType, accountDataList: accountDataList)
    ...
    private static func generateChartDataForEachWatchList() async {
        ...
        for watch in watchList {
            let chartDataListResult = generateChartDataForWatchAccount(id: watch.id!, accountIDList: watch.accountID)
            shared.chartDataList.updateValue(chartDataListResult, forKey: watch.id!)
        }
    }
    
    private static func generateChartDataForWatchAccount(id: String, accountIDList: [String]) -> [ChartData] {
    ...
    private static func generateChartDataForMultipleNonBrokerAccount(accountType: String, accountDataList: [AccountData]) async -> [ChartData] {
        var chartDataListResult = [ChartData]()
        var dummy = [[ChartData]]()
        for accountData in accountDataList {
            let chartData = shared.chartDataList.first(where: {
    ...
    private static func generateChartDataForMultipleBrokerAccounts(accountType: String, accountDataList: [AccountData]) async -> [ChartData] {
        var chartDataListResult = [ChartData]()
        var dummy = [[ChartData]]()
    ...
    private static func generateChartdataForMixedAccounts(accountType: String, accountDataList: [AccountData]) async -> [ChartData] {
        var chartDataListResult = [ChartData]()
        ...
        let chartDataForNonBrokerAccountList = await generateChartDataForMultipleNonBrokerAccount(accountType: accountType, accountDataList: nonBrokerAccountList)
        let chartDataForBrokerAccountList = await generateChartDataForMultipleBrokerAccounts(accountType: accountType, accountDataList: brokerAccountList)
